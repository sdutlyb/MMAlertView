...
- (id)init
{
    ...
    if (self) {
        ...
        _titleTextFont = [YBAlertViewConfig sharedConfig].titleTextFont;
...
- (void)addActionButtonsToBottom:(UIView *)bottomBar {
    ...
    for (int i = 0; i < [_buttonTitles count]; i++) {
        ...
        // 先确定按钮色颜色、字体等样式
        UIColor *titleColor = [YBAlertViewConfig sharedConfig].actionButtonTextColor;
        UIFont *buttonTitleFont = [YBAlertViewConfig sharedConfig].actionButtonFont;
        ...
        if (isPreferredAction) {
            titleColor = [YBAlertViewConfig sharedConfig].preferredActionButtonTextColor;
            buttonTitleFont = [YBAlertViewConfig sharedConfig].preferredActionButtonFont;
        }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ([touch.view isKindOfClass:[YBAlertView class]]) {
        [self close];
    }
}
